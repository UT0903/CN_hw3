...
int main(int argc, char* argv[]){
    ...
    if(argc != 5){
        fprintf(stderr,"usage: ./agent <sender port> <agent port> <recv port> <loss_rate>\n");
        exit(1);
    } else {
        setIP(ip[0], string("local").c_str());
        setIP(ip[1], string("local").c_str());
        setIP(ip[2], string("local").c_str());

        sscanf(argv[1], "%d", &port[0]);
        sscanf(argv[2], "%d", &port[1]);
        sscanf(argv[3], "%d", &port[2]);

        sscanf(argv[4], "%f", &loss_rate);
    }
